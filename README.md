...
Settings Sync can use a regular Git repository instead of a GitHub Gist. Open the extension settings page and set the following global settings:
...
The remote may point to GitHub, GitLab, an enterprise server, or a local bare repository. Upload commits the current User folder and pushes the selected branch. Download fetches the branch and replaces the working tree with the remote version. Use separate branches such as `master` and `office` as sync profiles. `syncLocalSettings.json`, `sync.lock`, ignored files and ignored folders are stored in `.git/info/exclude`, so tokens and local metadata are not committed. Set `exportType` back to `githubGist` to use the existing Gist flow.
